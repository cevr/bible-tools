...
function GroupText({
  ...
  const defaultProps = {
    ...
    className: cn(
      "border-border bg-muted dark:bg-input/64 relative inline-flex items-center rounded-lg border bg-clip-padding px-[calc(--spacing(3)-1px)] text-sm font-medium whitespace-nowrap shadow-xs transition-shadow outline-none before:pointer-events-none before:absolute before:inset-0 before:rounded-[calc(var(--radius-lg)-1px)] before:shadow-[0_1px_--theme(--color-black/4%)] dark:before:shadow-[0_-1px_--theme(--color-white/8%)] [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
      className,
    ),
  };
  return useRender({
